...
const ReleasePostCard: React.FC<ReleasePostCardProps> = ({
  ...
  <Card className={`group overflow-hidden hover:shadow-lg transition-all duration-200 cursor-pointer relative ${cardClass}`}>
    ...
    {isAdmin && (
      <Button
        ...
        title="Delete post"
      >
        <Trash2 className={`w-4 h-4 ${textClass}`} />
      </Button>
    )}
    ...
    <div onClick={() => onClick(post)} className="h-full">
      ...
      <CardContent className="p-6">
        <h4 className={`font-semibold mb-3 line-clamp-2 text-base leading-tight ${textClass}`}>
          {post.title}
        </h4>
        ...
        {/* Date section */}
        <div className={`flex items-center justify-between text-sm text-muted-foreground ${textClass}`}>
          <div className="flex items-center space-x-2">
            <Calendar className={`w-3 h-3 ${textClass}`} />
